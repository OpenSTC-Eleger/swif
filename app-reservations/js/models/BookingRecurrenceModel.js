	...
	var bookingRecurrence = GenericModel.extend({
		...
		getStartDate: function(type){
			...
			if(date !== false){
				...
				switch(type){
					case 'human':
						returnVal = moment.utc(date).local().format('LLL');
						break;
					default:
						returnVal = date;
						break;
		...
		getUntil: function(type){
			if(this.get('date_end') !== false){
				...
				switch(type){
					case 'human':
						returnVal = AppHelpers.convertDateToTz(this.get('date_end')).format('LL');
						break;
					default:
						returnVal = this.get('date_end');
						break;
				}
				return returnVal;
			}
		...
		getWeekdays: function(){
			...
			_.each(this.weekdays_vals, function(weekday){
				...
				if(data.length > 0){
					...
					_.each(data, function(date){
						...
						if(!date_start.isSame(resa_date_start)){
							...
							_.each(collectionLine.models,function(line){
		...
		fetchOccurrences: function(){
			...
			this.occurrences.fetch({
				data:{ filters:{ 0: {field:'recurrence_id.id',operator:'=',value:this.getId()}, 1: {field:'id', operator: '!=', value: this.getTemplate().getId()} }}
			}).done(function(){
		...
		persistentDestroyOnBackend: function(){
			...
			_.each(occurrenceToRemoveIds,function(id){
