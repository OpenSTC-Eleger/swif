...
], function(app, AppHelpers, TasksCollection, ItemInterventionTaskView, ModalInterventionAddTaskView){
...
//				});
				...
				if (!_.isUndefined(self.tasksCollection)) {
					...
					_.each(self.tasksCollection.models, function (task) {
						var itemInterventionTaskView = new ItemInterventionTaskView({model: task});
						$(self.el).find('#row-nested-objects').append(itemInterventionTaskView.render().el);
						self.tasksCollection.off();						
						//self.listenTo(task, 'add', this.add);
						//self.listenTo(tak, 'remove', this.destroyTask);
						//self.listenTo(this.options.inter,'change',this.changeInter);
