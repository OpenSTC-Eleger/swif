...
(function() {
  ...
  generateFile = function(path, data, callback) {
    return fs_utils.exists(path, function(exists) {
      var parentDir, write;
      if (exists) {
  ...
  scaffoldFile = function(rollback, from, to, templateData, parentDir, callback) {
    var _ref;
    _ref = [sysPath.join(parentDir, from), sysPath.join(parentDir, to)], from = _ref[0], to = _ref[1];
    if (rollback) {
  ...
  module.exports = scaffold = function(rollback, options, callback) {
    ...
    return helpers.loadPackages(helpers.pwd(), function(error, packages) {
      ...
      return generateFiles(rollback, generators, type, templateData, parentDir, function(error) {
        if (error != null) {
          return logger.error(error);
        }
        return callback();
